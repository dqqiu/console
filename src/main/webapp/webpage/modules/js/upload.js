...
var uploaders = new plupload.Uploader({ //实例化一个plupload上传对象
	...
	url : host,
	flash_swf_url : common.ctx+'/static/plupload/plupload/Moxie.swf',
	silverlight_xap_url : common.ctx+'/static/plupload/plupload/Moxie.xap',
	init: {
		...
		FileUploaded: function(uploader,file,responseObject) {
			if(responseObject.status==200){
				...
				if(uploaders.total.queued==0){
					...
					/*$.messager.show({
						title:'上传成功',
						msg:'文件上传完成,总共上传'+uploaders.total.uploaded+'个文件',
						showType:'fade',
						style:{
							right:'',
							bottom:''
					...
					/**由于上传中涉及到异步，所以把充值动作放在所有文件上传完成后执行**/
					recharge.option={
							amount: $("#money").val(),
							comment: $("#message").val(),
							createdBy: $("#userId").val(),
							fileName: upload.variable.fileName,
							mtTenantId: $("#tenantIdxxx").val(),
