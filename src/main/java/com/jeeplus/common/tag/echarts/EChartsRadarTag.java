...
public class EChartsRadarTag extends BodyTagSupport {
  ...
  public int doEndTag() throws JspException {
    ...
    /**
     * polar: [ { indicator: [ { text: '正北（N）', max: 100 }, { text: '西北（NW）', max: 100 }, { text:
     * '正西（W）', max: 100 }, { text: '西南（SW）', max: 100 }, { text: '正南（S）', max: 100 }, { text:
     * '东南（SE）', max: 100 }, { text: '正东（E）', max: 100 }, { text: '东北（NE）', max: 100 } ] } ]
     */
    // 工具栏
     ...
     * <c:forEach var="item" items="${towerList}" varStatus="status"> '${item.tower_mater}米风向',
     * </c:forEach> ] },
     */
